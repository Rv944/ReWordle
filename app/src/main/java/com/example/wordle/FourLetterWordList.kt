...
class FourLetterWordList : AppCompatActivity() {
    ...
    private lateinit var guessCk: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        guessCk = findViewById(R.id.guessCk)
        guessBtn = findViewById(R.id.guessBtn)
        correctWord = findViewById(R.id.correctWord)

        guessBtn.setOnClickListener {
            val guess = guessCk.text.toString().uppercase(Locale.ROOT)
            if (chancesLeft > 0) {
                val result = checkGuess(guess)
                correctWord.text = result
                chancesLeft--
                if (result == "0000") {
